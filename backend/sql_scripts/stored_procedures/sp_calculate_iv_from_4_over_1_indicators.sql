drop procedure if exists sp_calculate_iv_from_4_over_1_indicators;

set sql_mode = '';

DELIMITER $$
CREATE PROCEDURE `sp_calculate_iv_from_4_over_1_indicators`(in no varchar(200),
 in no1 varchar(200),
 in no2 varchar(200),
 in no3 varchar(200),
 in no4 varchar(200),
...
select iv1.year, @id, iv1.country_code,
    ...
    'not computable',
    (sum(case when i1.no_of_indicator in (no1, no2, no3, no4)
              then iv1.value
              end) 
    / sum(case when i1.no_of_indicator in(no5)
               then iv1.value
               end)) / divi.value) as value,  # Formula
'Added using stored procedure for creation of 1 indicator as ratio of 4 over 1 indicators' as additional_info,
