drop procedure if exists sp_calculate_iv_from_5_indicators_sum;

set sql_mode = '';

DELIMITER $$
CREATE PROCEDURE `sp_calculate_iv_from_5_indicators_sum`(in no varchar(200),
 in no1 varchar(200),
 in no2 varchar(200),
 in no3 varchar(200),
 in no4 varchar(200),
...
select iv1.year, @id, iv1.country_code, 
    ...
    'not computable',
    #sum(if(not iv1.value REGEXP '^[0-9]*[.]?[0-9]+$', 0, iv1.value))) as value,  # Formula - solution if sql_mode='' is not settable
    sum(iv1.value) / divi.value) as value,  # Formula
'Added using stored procedure for creation of 1 indicator as sum of 5 indicators' as additional_info,
...
and i1.no_of_indicator in(no1, no2, no3, no4, no5)
