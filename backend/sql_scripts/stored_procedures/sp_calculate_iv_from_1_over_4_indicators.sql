drop procedure if exists sp_calculate_iv_from_1_over_4_indicators;

set sql_mode = '';

DELIMITER $$
CREATE PROCEDURE `sp_calculate_iv_from_1_over_4_indicators`
(in no varchar(200),
 in no1 varchar(200),
 in no2 varchar(200),
 in no3 varchar(200),
...
select iv1.year, @id, iv1.country_code,
    if(exists (select * from indicators_values eiv
               ...
               where 1=1
			   and ei.no_of_indicator in(no1, no2, no3, no4, no5)
    ...
    'not computable',
    (sum(case when i1.no_of_indicator in (no1)
              then iv1.value
              end) 
    / sum(case when i1.no_of_indicator in(no2, no3, no4, no5)
               then iv1.value
               end)) / divi.value) as value,  # Formula
'Added using stored procedure for calculation of indicator values as ratio of 1 over 4 indicators' as additional_info,
...
#call sp_calculate_iv_from_1_over_6_indicators('', '', 'consolidated-a', 1);
